...
class Map:
    def __init__(self, width, height):
        ...
        # Адаптируем количество элементов в зависимости от размера карты
        base_size = 20 * 10
        current_size = width * height
        size_factor = current_size / base_size
        
        self.generate_vegetation(5, 10)
        river_count = max(2, round(2 * size_factor))
        for _ in range(river_count):
            self.generate_water_path(10 * size_factor)
